  ...
  app.get('/api/UWDC/', (req, res) => {
    ...
    axios.get('https://content-out.bepress.com/v2/digitalcommons.law.uw.edu/query?limit=5&q=' + DCuserrequest,{
      ...
      .then(function (response) {
        ...
        let capresp4 = capresp3.replace(/"download_format"[\s]*\][\s]*\}/m, '"download_format"]}')
        let capresp5 = capresp4.replace('"start"','"next"')
        let DCheader = capresp5.match(/"total[\s\S]*mat\"[\s]*\][\s]*\}/m)
